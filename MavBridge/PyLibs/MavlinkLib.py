...
latitudeOrigin = 515001180
longitudeOrigin = -25473940
altitudeOrigin = 10000000

localpositionnedcoords  = [0,0,0]
globalpositionintcoords = [0,0]
yawcoords = 0

def SimulateEstPosition():
    global _estPosOffset
    _estPosOffset += 0.01

    # Wrap
    if(_estPosOffset > 3.0):
        _estPosOffset = -3.0
    #end if 
    return _estPosOffset
# end def
...
class Mavlink(object):
    ...
    def DecodeModeFlags(self):
        ...
        if(self.heartbeat.custom_mode==16):
            statusText += "Initialising"
        if(self.heartbeat.custom_mode==5):
            statusText += "FBWA"
        if(self.heartbeat.custom_mode==6):
            statusText += "FBWB"
        if(self.heartbeat.custom_mode==17):
            statusText += "QStabilize"
        elif(self.heartbeat.custom_mode==18):
            statusText += "QHover"
        elif(self.heartbeat.custom_mode==19):
            statusText += "QLoiter"
        elif(self.heartbeat.custom_mode==20):
            statusText += "QLand"
        elif(self.heartbeat.custom_mode==21):
            statusText += "QRTL"
        else:
    ...
    def RcChannelSet(self, channel1, channel2, channel3, channel4, channel7, channel8, channel9):
        # Micmatch between channels here and on drone
        self.channel1 = channel3
        self.channel2 = channel4
        self.channel3 = channel2
        self.channel4 = channel1
        self.channel5 = 1500
        self.channel6 = 1500
